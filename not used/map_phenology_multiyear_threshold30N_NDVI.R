...
phenology_files<-list.files("./PROJECT/SIF_phenology/pheno_VI_hd_var_threshold/",pattern="30N",full.names = T)
...
retrieve_pheno_fix<-function(xt){
  ...
  x[is.nan(x)]<-NA
  x[x< -5000]<-NA
  if (sum(is.na(x))>21|is.na(thresh_val)){
    return(c(-9999,-9999,-9999))
  }
...
for(i in 1:length(phenology_files)){
  year<-substr(basename(phenology_files[i]),12,15)
...
for(i in 1:length(year_files)){
  ...
  outfile<-paste("./PROJECT/SIF_phenology/pheno_VI_hd_fixed_threshold/SOS_EOS_30N_threshold_0.25_",year,".nc",sep="")
