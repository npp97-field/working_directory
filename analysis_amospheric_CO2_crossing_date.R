...
smoothed_resi<-gaussian_filter(ts = resid,fwhm = 30)
...
deseasonal<-gaussian_filter(c(smoothed_brw,rep(smoothed_brw[(length(smoothed_brw)-364):length(smoothed_brw)],3)), 
                            fwhm=390)
deseasonal<-deseasonal[1:length(smoothed_brw)]
...
fzc<-rep(NA,37)
for (y in 1980:2016){
  dat<-co2_brw$seasonal[co2_brw$year==y]
